...
MidiService.prototype.selectChannel = function(channel) {
    var bytes = this.mappingEngine.buttonPacket("select", channel, true);
    this.sendSysEx(bytes);
    return { sent: true, action: "selectChannel", channel: channel, sysex: yamaha01vSysex.formatBytes(bytes) };
};
