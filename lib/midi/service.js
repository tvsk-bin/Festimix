...
MidiService.prototype.selectMasterBus = function(mode) {
    var packets = this.mappingEngine.selectMasterBusPackets(mode);
    var self = this;
    var sent = packets.map(function(bytes) {
        self.sendSysEx(bytes);
        return yamaha01vSysex.formatBytes(bytes);
    });
    return { sent: sent.length > 0, action: "selectMasterBus", mode: mode, sysex: sent };
};
...
MidiService.prototype.mapIncomingToUi = function(msg) {
    return this.mappingEngine.incomingEvents(msg, this.channel);
};
