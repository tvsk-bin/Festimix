...
MappingEngine.prototype.auxPrePostPackets = function(auxId, mode) {
    ...
    var navigationFamily = parseByte(config.navigationFamily || "0x23");
    var buttonFamily = parseByte(config.buttonFamily || "0x46");
    var navigationPage = config.navigationPage && config.navigationPage[auxKey];
    if (navigationPage === undefined || navigationPage === null) throw new Error("Missing AUX pre/post navigation page: " + auxId);
    ...
    for (var channel = 0; channel < channelCount; channel++) {
        ...
        packets.push(header.concat([0x04, navigationFamily, 0x01, channel, parseByte(navigationPage), 0x00, 0xf7]));
        packets.push(header.concat([0x04, buttonFamily, address[0], address[1], pressBit + index, 0xf7]));
        packets.push(header.concat([0x04, buttonFamily, address[0], address[1], releaseBase + index, 0xf7]));
    }
    return packets;
};
