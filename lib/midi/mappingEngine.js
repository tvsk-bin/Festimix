...
MappingEngine.prototype.selectChannelPackets = function(channel) {
    ...
    function packet(keyNo, data) {
        return header.concat([0x04, 0x43, 0x00, parseByte(keyNo), parseByte(data), 0xf7]);
    }

    if (entry) {
        return [packet(entry.keyNo, entry.press), packet(entry.keyNo, entry.release)];
    }

    var numeric = parseInt(channel, 10);
    if (!isNaN(numeric) && numeric >= 1 && numeric <= 16) {
        var keyNo = numeric <= 8 ? "0x64" : "0x65";
        var index = (numeric - 1) % 8;
        return [packet(keyNo, 0x08 + index), packet(keyNo, index)];
    }
