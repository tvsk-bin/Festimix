...
MappingEngine.prototype.selectChannelPackets = function(channel) {
    ...
    function packet(channelCode) {
        return String(config.family || "F0 43 10 3E 04 23 01 channelCode 01 04 F7").trim().split(/\s+/).filter(Boolean).map(function(byte) {
            return byte === "channelCode" ? parseByte(channelCode) : parseByte(byte);
        });
    }

    if (entry) {
        if (entry.bytes) return [parseByteString(entry.bytes)];
