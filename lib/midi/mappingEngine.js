...
function familyMatchesPacket(family, packet) {
    var tokens = String(family).trim().split(/\s+/).filter(Boolean);
    if (tokens.length !== packet.length) {
        if (!(tokens[tokens.length - 1] && parseByte(tokens[tokens.length - 1]) === 0xf7 && tokens.length === packet.length + 1)) {
            return false;
        }
        tokens = tokens.slice(0, -1);
    }
    for (var i = 0; i < tokens.length; i += 1) {
        if (tokens[i] === "data") continue;
        if (i === 2 && parseByte(tokens[0]) === 0xf0 && parseByte(tokens[1]) === 0x43) {
            if ((parseByte(tokens[i]) & 0x0f) !== (packet[i] & 0x0f)) return false;
            continue;
        }
        if (parseByte(tokens[i]) !== packet[i]) return false;
    }
    return true;
}

function packetDataByte(packet) {
    return packet[packet.length - 1] === 0xf7 ? packet[packet.length - 2] : packet[packet.length - 1];
}
...
function uiChannelId(key) {
    var normalized = channelKey(key);
    if (normalized.indexOf("CH") === 0) return normalized.slice(2);
    if (normalized === "RTN1" || normalized === "RTN2") return normalized;
    return normalized;
}

function uiMasterId(key) {
    var normalized = masterKey(key);
    if (normalized === "STEREO") return "stereo";
    if (normalized === "EFFECT1") return "effect1";
    if (normalized === "EFFECT2") return "effect2";
    return normalized.toLowerCase();
}

function rawEqControlKey(band, parameter) {
    var bandKey = {
        LOW: "low",
        LO_MID: "lowMid",
        HI_MID: "highMid",
    ...
    if (parameter === "FREQ") return "freq";
    if (parameter === "GAIN") return "gain";
    if (parameter === "Q" || parameter === "Q_OR_TYPE") return "q";
    return null;
}
...
MappingEngine.prototype.selectChannelPackets = function(channel) {
    if (this.mapping.channelSelectRemote && this.mapping.channelSelectRemote.sendToConsole === false) return [];
    var key = channelKey(channel);
    if (key === "CH13" || key === "CH14") key = "CH13_14";
    if (key === "CH15" || key === "CH16") key = "CH15_16";
    var observed = this.mapping.buttonSysex && this.mapping.buttonSysex.selectObserved || {};
    var observedEntry = observed[key];
    var config = this.mapping.channelSelectRemote || {};
    var entry = config.targets && config.targets[key];

    function packetFromFamily(family, channelCode) {
        return String(family || "F0 43 10 3E 04 23 01 channelCode 10 00 F7").trim().split(/\s+/).filter(Boolean).map(function(byte) {
            return byte === "channelCode" ? parseByte(channelCode) : parseByte(byte);
        });
    }

    function packet(channelCode) {
        return packetFromFamily(observed.family, channelCode);
    }

    if (config.sendFamily === "channelSelectRemote" && entry && entry.channelCode !== undefined) {
        return [packetFromFamily(config.family, entry.channelCode)];
    }

    if (observedEntry) {
        if (observedEntry.bytes) return [parseByteString(observedEntry.bytes)];
        if (observedEntry.channelCode !== undefined) return [packet(observedEntry.channelCode)];
    }

    if (entry) {
        if (entry.channelCode !== undefined) return [packet(entry.channelCode)];
    }

    var numeric = parseInt(channel, 10);
    if (!isNaN(numeric) && numeric >= 1 && numeric <= 16) {
        if (config.sendFamily === "channelSelectRemote") return [packetFromFamily(config.family, numeric - 1)];
        return [packet(numeric - 1)];
    }

    return [];
};

MappingEngine.prototype.selectMasterBusPackets = function(mode) {
    var config = this.mapping.masterBusSelectRemote || {};
    if (config.sendToConsole === false) return [];
    var key = String(mode || "").toUpperCase();
    var sequence = config.sequences && config.sequences[key];
    if (!Array.isArray(sequence)) return [];
    return sequence.map(parseByteString);
};
...
MappingEngine.prototype.auxPrePostPackets = function(auxId, mode) {
    ...
    var settingFamily = parseByte(config.settingFamily || "0x40");
    var preValueBase = parseByte(config.preValueBase || "0x08");
    var header = this.mapping.sysex.yamahaHeader.map(function(byte) {
        return byte === "1n" ? 0x10 : parseByte(byte);
    });
    var packets = [];
    ...
    for (var channel = 0; channel < channelCount; channel++) {
        ...
        packets.push(header.concat([0x04, settingFamily, address[0], address[1], preValueBase + index, 0xf7]));
    }
    return packets;
};
...
MappingEngine.prototype.parameterMessage = function(parameterId, value) {
    var parts = String(parameterId).split(".");
    if (parts[0] === "channelFader" && this.mapping.sysexParameters && this.mapping.sysexParameters.channelFader) {
        var channelFaderKey = channelKey(parts[1]);
        var channelFaderAddress = this.mapping.sysexParameters.channelFader[channelFaderKey];
        if (channelFaderAddress) {
            var channelFaderAddr = addressBytes(channelFaderAddress);
            return {
                type: "sysex",
                parameterId: parameterId,
                bytes: this.familyPacket("editBuffer7bitWrite", [channelFaderAddr[0], channelFaderAddr[1], parseByte(value)])
            };
        }
    }
    ...
    if ((parts[0] === "fx1Send" || parts[0] === "fx2Send") &&
        parts.length === 2 &&
        this.mapping.effectSends) {
        var effectSendKey = channelKey(parts[1]);
        var effectSendAddress = this.mapping.effectSends.sends &&
            this.mapping.effectSends.sends[parts[0]] &&
            this.mapping.effectSends.sends[parts[0]][effectSendKey];
        if (effectSendAddress) {
            var effectSendAddr = addressBytes(effectSendAddress);
            return {
                type: "sysex",
                parameterId: parameterId,
                bytes: this.familyPacket("editBuffer7bitWrite", [effectSendAddr[0], effectSendAddr[1], parseByte(value)])
            };
        }
    }
    ...
    if (parts[0] === "channelOn" && this.mapping.buttonSysex && this.mapping.buttonSysex.onOffObserved) {
        ...
        if (observedOn && observedOn.values) {
            var observedOnFamily = observedOn.family || this.mapping.buttonSysex.onOffObserved.family;
            return {
                type: "sysex",
                parameterId: parameterId,
    ...
    if (parts[0] === "masterOn" && this.mapping.buttonSysex && this.mapping.buttonSysex.masterOnObserved) {
        ...
        if (masterOn && masterOn.values) {
            var masterOnFamily = masterOn.family || this.mapping.buttonSysex.masterOnObserved.family;
            return {
                type: "sysex",
                parameterId: parameterId,
                bytes: observedFamilyPacket(
...
MappingEngine.prototype.reverseCcEvent = function(cc, value) {
    var ccMappings = this.mapping.ccMappings || {};
    var groupNames = ["channelFader", "masterFader", "channelOn", "masterOn", "pan", "fx1Send", "fx2Send"];
    for (var groupIndex = 0; groupIndex < groupNames.length; groupIndex += 1) {
        var group = groupNames[groupIndex];
        var map = ccMappings[group] || {};
        var keys = Object.keys(map);
        for (var keyIndex = 0; keyIndex < keys.length; keyIndex += 1) {
            var key = keys[keyIndex];
            if (parseByte(map[key]) !== cc) continue;
            var event = {
                transport: "cc",
                group: group,
                target: key,
            ...
            if (group === "channelFader" || group === "channelOn" || group === "pan" || group === "fx1Send" || group === "fx2Send") {
                event.channelId = uiChannelId(key);
            }
            if (group === "masterFader" || group === "masterOn") {
                event.masterId = uiMasterId(key);
            }
            if (group === "channelOn" || group === "masterOn") {
                event.enabled = event.value >= 64;
            }
            return event;
        }
    }
...
MappingEngine.prototype.reverseAddressEvent = function(address, value, familyId) {
    var mapping = this.mapping;
    var key;
    var channelFaders = (mapping.sysexParameters && mapping.sysexParameters.channelFader) || {};
    for (key in channelFaders) {
        if (parseNumber(channelFaders[key]) === address) {
            return {
                transport: "sysex",
                family: familyId,
                group: "channelFader",
                target: key,
                channelId: uiChannelId(key),
                value: parseByte(value),
                valueMax: 127
            };
        }
    ...
    var masterFaders = (mapping.sysexParameters && mapping.sysexParameters.masterFader) || {};
    for (key in masterFaders) {
        if (parseNumber(masterFaders[key]) === address) {
            return {
                transport: "sysex",
                family: familyId,
                group: "masterFader",
                target: key,
                masterId: uiMasterId(key),
                value: parseByte(value),
                valueMax: 127
            };
        }
    ...
    var auxSends = (mapping.auxSends && mapping.auxSends.sends) || {};
    Object.keys(auxSends).forEach(function(auxKey) {
        Object.keys(auxSends[auxKey] || {}).forEach(function(channel) {
            if (parseNumber(auxSends[auxKey][channel]) === address) {
                key = { aux: auxKey, channel: channel };
            }
        });
    });
    if (key && key.aux) {
        return {
            ...
            aux: key.aux,
            target: key.channel,
            channelId: uiChannelId(key.channel),
            value: parseByte(value),
            valueMax: 127
        };
    }

    var effectSends = (mapping.effectSends && mapping.effectSends.sends) || {};
    var effectSendMatch = null;
    Object.keys(effectSends).forEach(function(group) {
        Object.keys(effectSends[group] || {}).forEach(function(channel) {
            if (parseNumber(effectSends[group][channel]) === address) {
                effectSendMatch = { group: group, channel: channel };
            }
        });
    });
    if (effectSendMatch) {
        return {
            transport: "sysex",
            family: familyId,
            group: effectSendMatch.group,
            target: effectSendMatch.channel,
            channelId: uiChannelId(effectSendMatch.channel),
            value: parseByte(value),
            valueMax: 127
        };
    }

    var pan = (mapping.sysexParameters && mapping.sysexParameters.pan) || {};
    for (key in pan) {
        if (parseNumber(pan[key]) === address) {
            return {
                transport: "sysex",
                family: familyId,
                group: "pan",
                target: key,
                channelId: uiChannelId(key.replace(/_(L|R)$/, "")),
                side: /_L$/.test(key) ? "left" : (/_R$/.test(key) ? "right" : null),
                value: parseByte(value),
                valueMax: 32
            };
        }
    ...
    var attenuators = (mapping.sysexParameters && mapping.sysexParameters.attenuator) || {};
    for (key in attenuators) {
        if (!attenuators[key] || typeof attenuators[key] !== "object" || Array.isArray(attenuators[key])) continue;
        var sides = Object.keys(attenuators[key]);
        for (var sideIndex = 0; sideIndex < sides.length; sideIndex += 1) {
            var side = sides[sideIndex];
            if (typeof attenuators[key][side] !== "string") continue;
            if (parseNumber(attenuators[key][side]) === address) {
                return {
                    transport: "sysex",
                    family: familyId,
                    group: "attenuator",
    ...
    var rawEqEvent = this.reverseRawEqEvent(address, value, familyId);
    if (rawEqEvent) return rawEqEvent;

    return null;
};

MappingEngine.prototype.reverseRawEqEvent = function(address, value, familyId) {
    function fromTable(table, target, isMaster) {
        var bands = ["LOW", "LO_MID", "HI_MID", "HIGH"];
        var parameters = ["FREQ", "GAIN", "Q", "Q_OR_TYPE"];
        for (var bandIndex = 0; bandIndex < bands.length; bandIndex += 1) {
            var band = bands[bandIndex];
            var bandMap = table && table[band];
            if (!bandMap) continue;
            for (var parameterIndex = 0; parameterIndex < parameters.length; parameterIndex += 1) {
                var parameter = parameters[parameterIndex];
                if (!bandMap[parameter]) continue;
                if (parseNumber(bandMap[parameter]) !== address) continue;
                return {
                    transport: "sysex",
                    family: familyId,
                    group: "rawEq",
                    target: target,
                    channelId: isMaster ? null : uiChannelId(target),
                    masterId: isMaster ? uiMasterId(target) : null,
                    band: band,
                    parameter: parameter,
                    control: rawEqControlKey(band, parameter),
                    value: parseByte(value),
                    valueMax: 127
                };
            }
    ...
    var channelTables = this.mapping.eqExpandedCh1toCh8 || {};
    var channelKeys = Object.keys(channelTables).filter(function(key) {
        return key !== "status" && key !== "notes";
    });
    for (var i = 0; i < channelKeys.length; i += 1) {
        var channelEvent = fromTable(channelTables[channelKeys[i]], channelKeys[i], false);
        if (channelEvent) return channelEvent;
    }

    var targetTables = this.mapping.eqExpandedTargets || {};
    var targetKeys = Object.keys(targetTables).filter(function(key) {
        return key !== "status" && key !== "notes";
    });
    for (var j = 0; j < targetKeys.length; j += 1) {
        var targetKey = targetKeys[j];
        var targetEvent = fromTable(targetTables[targetKey], targetKey, targetKey !== "RTN1" && targetKey !== "RTN2");
        if (targetEvent) return targetEvent;
    }
    return null;
};

MappingEngine.prototype.reverseEffectReturnSendEvent = function(bytes) {
    var effectReturnSend = (this.mapping.sysexParameters && this.mapping.sysexParameters.effectReturnSend) || {};
    if (bytes.length < 10 || bytes[5] !== 0x30) return null;
    var prefix = [bytes[5], bytes[6], bytes[7]];
    var value = bytes[8];
    var sections = ["master", "eff1", "eff2", "aux1", "aux2", "aux3", "aux4", "fx1Send", "fx2Send"];
    for (var sectionIndex = 0; sectionIndex < sections.length; sectionIndex += 1) {
        var section = sections[sectionIndex];
        var targets = effectReturnSend[section] || {};
        var keys = Object.keys(targets);
        for (var keyIndex = 0; keyIndex < keys.length; keyIndex += 1) {
            var key = keys[keyIndex];
            var configured = configuredPrefixBytes(targets[key]);
            if (!configured || configured.length !== prefix.length) continue;
            if (configured[0] === prefix[0] && configured[1] === prefix[1] && configured[2] === prefix[2]) {
                return {
                    transport: "sysex",
                    family: "editBuffer7bitWrite",
                    group: "effectReturnSend",
                    bus: section,
                    target: key,
                    channelId: key === "effRtn1" ? "RTN1" : (key === "effRtn2" ? "RTN2" : null),
                    value: parseByte(value),
                    valueMax: 127
                };
            }
...
MappingEngine.prototype.reverseObservedButtonEvent = function(bytes) {
    var buttonSysex = this.mapping.buttonSysex || {};
    var key;
    var observedOn = buttonSysex.onOffObserved || {};
    for (key in observedOn) {
        if (!observedOn[key] || !observedOn[key].values) continue;
        var onFamily = observedOn[key].family || observedOn.family;
        if (!onFamily || !familyMatchesPacket(onFamily, bytes)) continue;
        var onValue = parseByte(observedOn[key].values[1]);
        var offValue = parseByte(observedOn[key].values[0]);
        if (bytes[bytes.length - 2] === onValue || bytes[bytes.length - 2] === offValue) {
            return {
                transport: "sysex",
                group: "channelOn",
                target: key,
                channelId: uiChannelId(key),
                enabled: bytes[bytes.length - 2] === onValue,
                value: bytes[bytes.length - 2],
                valueMax: 15
            };
        }
    }

    var observedMasterOn = buttonSysex.masterOnObserved || {};
    for (key in observedMasterOn) {
        if (!observedMasterOn[key] || !observedMasterOn[key].values) continue;
        var masterFamily = observedMasterOn[key].family || observedMasterOn.family;
        if (!masterFamily || !familyMatchesPacket(masterFamily, bytes)) continue;
        var masterOnValue = parseByte(observedMasterOn[key].values[1]);
        var masterOffValue = parseByte(observedMasterOn[key].values[0]);
        if (bytes[bytes.length - 2] === masterOnValue || bytes[bytes.length - 2] === masterOffValue) {
            return {
                transport: "sysex",
                group: "masterOn",
                target: key,
                masterId: uiMasterId(key),
                enabled: bytes[bytes.length - 2] === masterOnValue,
                value: bytes[bytes.length - 2],
                valueMax: 15
            };
        }
    ...
    var selectConfig = this.mapping.channelSelectRemote || {};
    if (selectConfig.receiveFromConsole !== false) {
        var observedSelect = buttonSysex.selectObserved || {};
        var selectTargets = Object.assign({}, selectConfig.targets || {}, observedSelect);
        for (key in selectTargets) {
            if (!selectTargets[key] || selectTargets[key].channelCode === undefined) continue;
            var selectCode = parseByte(selectTargets[key].channelCode);
            var isRuntimeSelect = bytes[8] === 0x01 && bytes[9] === 0x04;
            if (bytes[5] === 0x23 && bytes[6] === 0x01 && bytes[7] === selectCode && isRuntimeSelect) {
                return {
                    transport: "sysex",
                    group: "channelSelect",
                    target: key,
    ...
    var masterSoloEvent = this.reverseMasterSoloEvent(bytes);
    if (masterSoloEvent) return masterSoloEvent;

    if (bytes[5] === 0x61 && bytes[6] === 0x00 && bytes[7] === 0x0f) {
        var lowerSoloValue = packetDataByte(bytes);
        var lowerSoloTarget = ["CH1", "CH2", "CH3", "CH4", "CH5", "CH6", "CH7", "CH8"][lowerSoloValue % 8];
        if (lowerSoloTarget) {
            return {
                transport: "sysex",
                group: "channelSolo",
                target: lowerSoloTarget,
                channelId: uiChannelId(lowerSoloTarget),
                enabled: lowerSoloValue >= 0x08,
                value: lowerSoloValue,
                valueMax: 15
            };
        }
    }
    if (bytes[5] === 0x61 && bytes[6] === 0x00 && bytes[7] === 0x10) {
        var upperSoloValue = packetDataByte(bytes);
        var upperSoloTarget = ["CH9", "CH10", "CH11", "CH12", "CH13_14", "CH15_16", "RTN1", "RTN2"][upperSoloValue % 8];
        if (upperSoloTarget) {
            return {
                transport: "sysex",
                group: "channelSolo",
                target: upperSoloTarget,
                channelId: uiChannelId(upperSoloTarget),
                enabled: upperSoloValue >= 0x08,
                value: upperSoloValue,
                valueMax: 15
            };
        }
    }
    var observedSolo = buttonSysex.soloObserved || {};
    for (key in observedSolo) {
        if (!observedSolo[key] || !observedSolo[key].valuesA || !observedSolo.familyA) continue;
        if (!(bytes[5] === 0x61 && bytes[6] === 0x00 && bytes[7] === 0x10)) continue;
        var soloOnValue = parseByte(observedSolo[key].valuesA[1]);
        var soloOffValue = parseByte(observedSolo[key].valuesA[0]);
        var soloValue = packetDataByte(bytes);
        if (soloValue === soloOnValue || soloValue === soloOffValue) {
            return {
                transport: "sysex",
                group: "channelSolo",
                target: key,
                channelId: uiChannelId(key),
                enabled: soloValue === soloOnValue,
                value: soloValue,
                valueMax: 15
            };
        }
    ...
    var phaseObserved = buttonSysex.phaseObserved || {};
    var families = phaseObserved.families || {};
    var channels = phaseObserved.channels || {};
    for (key in channels) {
        var entries = channels[key] || [];
        for (var i = 0; i < entries.length; i += 1) {
            var entry = entries[i];
            var phaseFamily = families[entry.family] || entry.family;
            if (!phaseFamily || !familyMatchesPacket(phaseFamily, bytes)) continue;
            var phaseOnValue = parseByte(entry.values[1]);
            var phaseOffValue = parseByte(entry.values[0]);
            if (bytes[bytes.length - 2] === phaseOnValue || bytes[bytes.length - 2] === phaseOffValue) {
                return {
                    transport: "sysex",
                    group: "phase",
                    target: key,
                    channelId: uiChannelId(key),
                    enabled: bytes[bytes.length - 2] === phaseOnValue,
                    value: bytes[bytes.length - 2],
                    valueMax: 15
                };
            }
...
MappingEngine.prototype.reverseMasterSoloEvent = function(bytes) {
    if (bytes[5] !== 0x61 || bytes[6] !== 0x00) return null;
    var config = this.mapping.masterSoloMonitorOutAssign || {};
    var targets = config.targets || {};
    var value = packetDataByte(bytes);
    var key;
    for (key in targets) {
        var target = targets[key] || {};
        var onPacket = target.on ? parseByteString(target.on) : null;
        var offPacket = target.off ? parseByteString(target.off) : null;
        if (onPacket && bytes[7] === onPacket[7] && value === packetDataByte(onPacket)) {
            return {
                transport: "sysex",
                group: "masterSolo",
                target: key,
        ...
        if (offPacket && bytes[7] === offPacket[7] && value === packetDataByte(offPacket)) {
            return {
                transport: "sysex",
                group: "masterSolo",
                target: key,
...
MappingEngine.prototype.reverseKeyRemoteSoloEvent = function(bytes) {
    if (bytes.length < 9 || bytes[5] !== 0x43 || bytes[6] !== 0x00) return null;
    var keyNo = bytes[7];
    var data = bytes[8];
    if (keyNo !== 0x64 && keyNo !== 0x65) return null;
    var isPress = data >= 0x08 && data <= 0x0f;
    var isRelease = data <= 0x07;
    if (!isPress && !isRelease) return null;
    var bankTargets = {
        0x64: ["CH1", "CH2", "CH3", "CH4", "CH5", "CH6", "CH7", "CH8"],
        0x65: ["CH9", "CH10", "CH11", "CH12", "CH13_14", "CH15_16", "RTN1", "RTN2"]
    };
    var target = bankTargets[keyNo] && bankTargets[keyNo][data % 8];
    if (!target) return null;
    return {
        transport: "sysex",
        group: "channelSolo",
        target: target,
        channelId: uiChannelId(target),
        toggle: isPress,
        release: isRelease,
        value: data,
        valueMax: 15
...
MappingEngine.prototype.incomingEvents = function(msg, midiChannel) {
    if (!msg || msg.length < 3) return [];
    var bytes = Array.prototype.slice.call(msg).map(parseByte);
    var status = bytes[0] & 0xf0;
    var channel = (bytes[0] & 0x0f) + 1;
    if (status === 0xb0) {
        if (midiChannel && channel !== midiChannel) return [];
        var ccEvent = this.reverseCcEvent(bytes[1], bytes[2]);
        return ccEvent ? [ccEvent] : [];
    }
    if (bytes[0] !== 0xf0 || bytes[1] !== 0x43 || bytes[3] !== 0x3e || bytes[4] !== 0x04) return [];

    var buttonEvent = this.reverseObservedButtonEvent(bytes);
    if (buttonEvent) return [buttonEvent];

    var effectReturnEvent = this.reverseEffectReturnSendEvent(bytes);
    if (effectReturnEvent) return [effectReturnEvent];

    if (bytes[5] === 0x30 && bytes.length >= 9) {
        var address = (bytes[6] << 7) + bytes[7];
        var addressEvent = this.reverseAddressEvent(address, bytes[8], "editBuffer7bitWrite");
        return addressEvent ? [addressEvent] : [];
    }
    if (bytes[5] === 0x00 && bytes.length >= 10) {
        var wideAddress = (bytes[6] << 7) + bytes[7];
        var wideValue = (bytes[8] << 7) + bytes[9];
        var wideEvent = this.reverseAddressEvent(wideAddress, wideValue, "editBufferWideWrite");
        return wideEvent ? [wideEvent] : [];
    }
    return [];
};
