...
function familyMatchesPacket(family, packet) {
    ...
    if (tokens.length !== packet.length) return false;
...
MappingEngine.prototype.selectChannelPackets = function(channel) {
    ...
    function packet(channelCode) {
        return String(observed.family || "F0 43 10 3E 04 23 01 channelCode 10 00 F7").trim().split(/\s+/).filter(Boolean).map(function(byte) {
...
MappingEngine.prototype.incomingEvents = function(msg, midiChannel) {
    ...
    if (bytes[5] === 0x30 && bytes.length >= 10) {
    ...
    if (bytes[5] === 0x00 && bytes.length >= 11) {
