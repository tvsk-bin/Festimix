...
function babyfaceLowcutHzToRaw(value) {
    var hz = clamp(value, 20, 500);
    var ratio = (Math.log(hz) - Math.log(20)) / (Math.log(500) - Math.log(20));
...
LogicalEngine.prototype.usesBabyfaceEqModel = function() {
    return this.profile === "rmeBabyfaceProFs12";
};
...
LogicalEngine.prototype.setEqIntent = function(channel, control, value, state) {
    ...
    if (this.usesBabyfaceEqModel()) {
        if (control === "hpf") {
            if (value === "75" || value === "100") {
                commands.push(this.writeEq(channel, "HPF", "Q_OR_TYPE", 3));
                commands.push(this.writeEq(channel, "HPF", "FREQ", babyfaceLowcutHzToRaw(value)));
            } else {
                commands.push(this.writeEq(channel, "HPF", "Q_OR_TYPE", 0));
            }
        }
        if (control === "lowGain") {
            commands.push(this.writeEq(channel, "LOW", "Q_OR_TYPE", 42));
            commands.push(this.writeEq(channel, "LOW", "Q", 5));
            commands.push(this.writeEq(channel, "LOW", "FREQ", this.constants.bassFixedFreq125));
            commands.push(this.writeEq(channel, "LOW", "GAIN", gainDbToByte(value)));
        }
        if (control === "midGain" || control === "midFrequency") {
            var babyfaceFreq = state.midFrequency || 250;
            var babyfaceMidGain = state.midGain !== undefined && state.midGain !== null ? state.midGain : (control === "midGain" ? value : 0);
            commands.push(this.writeEq(channel, "MID", "Q", 5));
            commands.push(this.writeEq(channel, "MID", "FREQ", this.constants.hiMidFreq[babyfaceFreq] || this.constants.hiMidFreq[250]));
            commands.push(this.writeEq(channel, "MID", "GAIN", gainDbToByte(babyfaceMidGain)));
        }
        if (control === "highGain") {
            commands.push(this.writeEq(channel, "HIGH", "Q_OR_TYPE", this.constants.highShelfMode));
            commands.push(this.writeEq(channel, "HIGH", "Q", 5));
...
LogicalEngine.prototype.setMasterEqIntent = function(target, control, value, state) {
    ...
    var commands = [];
    if (this.usesBabyfaceEqModel()) {
        var babyfaceControlMap = {
            lowGain: { band: "LOW", gainKey: "low", freqKey: "lowFrequency", defaultFreq: 80, typeParam: "Q_OR_TYPE", typeValue: 42 },
            lowFrequency: { band: "LOW", gainKey: "low", freqKey: "lowFrequency", defaultFreq: 80, typeParam: "Q_OR_TYPE", typeValue: 42 },
            lowMidGain: { band: "MID", gainKey: "lowMid", freqKey: "lowMidFrequency", defaultFreq: 1000, typeParam: "Q", typeValue: 3 },
            lowMidFrequency: { band: "MID", gainKey: "lowMid", freqKey: "lowMidFrequency", defaultFreq: 1000, typeParam: "Q", typeValue: 3 },
            highGain: { band: "HIGH", gainKey: "high", freqKey: "highFrequency", defaultFreq: 10000, typeParam: "Q_OR_TYPE", typeValue: 42 },
            highFrequency: { band: "HIGH", gainKey: "high", freqKey: "highFrequency", defaultFreq: 10000, typeParam: "Q_OR_TYPE", typeValue: 42 }
        };
        var babyfaceConfig = babyfaceControlMap[control];
        if (babyfaceConfig) {
            var babyfaceFreq = state[babyfaceConfig.freqKey] || babyfaceConfig.defaultFreq;
            var babyfaceGain = state[babyfaceConfig.gainKey] !== undefined && state[babyfaceConfig.gainKey] !== null ? state[babyfaceConfig.gainKey] : (control.indexOf("Gain") >= 0 ? value : 0);
            var babyfaceFreqRaw = constants.frequencies[babyfaceConfig.band] && constants.frequencies[babyfaceConfig.band][babyfaceFreq];
            var babyfaceHighOverrides = { 8000: 104, 10000: 109, 12000: 112 };
            if (babyfaceConfig.band === "HIGH" && babyfaceHighOverrides[babyfaceFreq] !== undefined) babyfaceFreqRaw = babyfaceHighOverrides[babyfaceFreq];
            var babyfaceMidOverrides = { 180: 39, 250: 43, 500: 55, 1000: 67, 2000: 79, 4000: 91, 6000: 98 };
            if (babyfaceConfig.band === "MID" && babyfaceMidOverrides[babyfaceFreq] !== undefined) babyfaceFreqRaw = babyfaceMidOverrides[babyfaceFreq];
            if (babyfaceFreqRaw === undefined) babyfaceFreqRaw = constants.frequencies[babyfaceConfig.band][babyfaceConfig.defaultFreq];
            commands.push(this.writeEq(targetKey, babyfaceConfig.band, babyfaceConfig.typeParam, babyfaceConfig.typeValue));
            commands.push(this.writeEq(targetKey, babyfaceConfig.band, "FREQ", babyfaceFreqRaw));
            commands.push(this.writeEq(targetKey, babyfaceConfig.band, "GAIN", gainDbToByte(babyfaceGain)));
        }
...
LogicalEngine.prototype.setRawYamahaEqIntent = function(target, state) {
    state = state || {};
    var pageId = target && target.type === "master" && target.id && target.id.indexOf("aux") === 0 ? "auxMasterPage" :
        target && target.type === "master" ? "mixMasterPage" : "channelPage";
    var module = this.moduleFor(pageId, "assistEq");
    var targetKey = target && target.type === "master" ? this.masterEqTargetKey(target) : (target && (target.midiKey || target.id));
    var commands = [];
    var bands = state.bands || {};
    var isBabyface = this.usesBabyfaceEqModel();
    var bandParams = isBabyface ? {
        HPF: "Q_OR_TYPE",
        LOW: "Q",
        MID: "Q",
        HIGH: "Q"
    ...
    (isBabyface ? ["HPF", "LOW", "MID", "HIGH"] : ["LOW", "LO_MID", "HI_MID", "HIGH"]).forEach(function(band) {
        var values = bands[band] || {};
        if (isBabyface && values.type !== undefined && (band === "LOW" || band === "HIGH")) {
            commands.push(this.writeEq(targetKey, band, "Q_OR_TYPE", values.type));
        }
        if (values.q !== undefined) {
            commands.push(this.writeEq(targetKey, band, bandParams[band], values.q));
        }
        if (values.freq !== undefined) commands.push(this.writeEq(targetKey, band, "FREQ", values.freq));
        if (values.gain !== undefined) commands.push(this.writeEq(targetKey, band, "GAIN", values.gain));
    }, this);
    return [this.moduleIntent(module, "eq.rawYamaha", { target: target, state: state }, commands)];
};
    ...
    if (this.usesBabyfaceEqModel()) {
        if (control === "hpf") {
            if (value === 75 || value === 100 || value === 175 || value === 250 || value === "75" || value === "100" || value === "175" || value === "250") {
                commands.push(this.writeEq(targetKey, "HPF", "Q_OR_TYPE", 3));
                var auxHpfFreq = babyfaceLowcutHzToRaw(value);
                commands.push(this.writeEq(targetKey, "HPF", "FREQ", auxHpfFreq));
            } else {
                commands.push(this.writeEq(targetKey, "HPF", "Q_OR_TYPE", 0));
            }
        }
        if (control === "lowNotchGain" || control === "lowNotchFreq") {
            var babyfaceLowGain = state.lowNotchInit ? 12 : (state.lowNotchGain === undefined ? 0 : state.lowNotchGain);
            commands.push(this.writeEq(targetKey, "LOW", "Q", 40));
            commands.push(this.writeEq(targetKey, "LOW", "FREQ", logHzToRaw(state.lowNotchFreq || 100, constants.lowNotchFreqRawRange)));
            commands.push(this.writeEq(targetKey, "LOW", "GAIN", auxGainDbToByte(babyfaceLowGain)));
        }
        if (control === "highNotchGain" || control === "highNotchFreq") {
            var babyfaceMidGain = state.highNotchInit ? 12 : (state.highNotchGain === undefined ? 0 : state.highNotchGain);
            commands.push(this.writeEq(targetKey, "MID", "Q", 40));
            commands.push(this.writeEq(targetKey, "MID", "FREQ", logHzToRaw(state.highNotchFreq || 800, constants.highNotchFreqRawRange)));
            commands.push(this.writeEq(targetKey, "MID", "GAIN", auxGainDbToByte(babyfaceMidGain)));
        }
        if (control === "presence") {
            var babyfacePresenceGain = state.presence === undefined ? value : state.presence;
            commands.push(this.writeEq(targetKey, "HIGH", "Q_OR_TYPE", this.constants.highShelfMode));
            commands.push(this.writeEq(targetKey, "HIGH", "FREQ", constants.presenceFreq5000));
            commands.push(this.writeEq(targetKey, "HIGH", "GAIN", gainDbToByte(babyfacePresenceGain)));
        }
